...
<template>
  <div mx-auto max-w-300>
    ...
    <div v-for="key, cidx in Object.keys(navigators)" :key="cidx" slide-enter :style="{ '--enter-stage': cidx + 1 }">
      <div
      ...
      <div mx-auto max-w-500 w-max py-2 grid="~ cols-1 sm:cols-2 md:cols-2 gap-4 lg:cols-3 xl:cols-4">
        <a
          v-for="item, idx in navigators[key]" :key="idx"
        ...
        >
          <div flex="~ items-center justify-center">
            <div pr-3>
              ...
              <VisActor v-else-if="item.icon === 'visactor'" class="text-4xl opacity-50" />
              <ChatGPT v-else-if="item.icon === 'chatgpt'" class="text-4xl opacity-50" />
              <img v-else-if="isUrl(item.icon || '')" :src="item.icon" class="min-w-11 w-11 rd-50%" :alt="item.name">
