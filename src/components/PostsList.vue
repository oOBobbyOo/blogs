...
import type { Post, RouterRecordProps } from '@/types'
...
const routes: Post[] = router.getRoutes()
  .filter((i: RouterRecordProps) => i.path.startsWith('/posts') && i.meta.frontmatter.date && !i.meta.frontmatter.draft)
  .filter((i: RouterRecordProps) => !i.path.endsWith('.html') && (i.meta.frontmatter.type || 'blog').split('+').includes(props.type))
  .map((i: RouterRecordProps) => ({
...
<template>
  <ul pl-0>
    ...
    <template v-for="route, idx in posts" :key="route.path">
      ...
      >
        <span absolute left--3rem top--2rem text-8em color-transparent font-bold text-stroke-2 text-stroke-hex-aaa op10>{{ getGroupName(route) }}</span>
      </div>
      <div
        class="slide-enter"
        :style="{
          ...
          <li class="no-underline" flex="~ col md:row gap-2 md:items-center">
            ...
            <div flex="~ gap-2 items-center">
              ...
              <span ws-nowrap text-sm op50>
                {{ formatDate(route.date, route.lang, true) }}
              </span>
